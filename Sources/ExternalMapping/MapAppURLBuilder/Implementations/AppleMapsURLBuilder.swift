...
enum AppleMapsURLBuilder {
    ...
    static func location(mapType: MapType? = nil,
        ...
        if let zoom = zoom, let zoomString = "\(zoomRange.clamp(value: zoom))".urlQueryEncoded {
    ...
    static func location(mapType: MapType? = nil,
        ...
        let url = location(mapType: mapType, query: query, address: address, near: near, coordinate: coordinate)
        var urlComponents = URLComponents(string: url.absoluteString)
        if let spanString = span?.stringValue.urlQueryEncoded {
    ...
    static func directions(to destinationAddress: String,
        ...
        var urlComponents = URLComponents(string: url.absoluteString)
        if let destinationAddress = destinationAddress.urlQueryEncoded {
            urlComponents?.addQueryItem(.init(parameter: .destinationAddress, value: destinationAddress))
        }
        if let startAddress = startAddress?.urlQueryEncoded {
            urlComponents?.addQueryItem(.init(parameter: .startAddress, value: startAddress))
        }
        if let transportationMode = transportationMode?.rawValue.urlQueryEncoded {
    ...
    static func search(query: String? = nil,
        ...
        if let location = location,
           let locationString = "\(location.latitude),\(location.longitude)".urlQueryEncoded {
        ...
        if let span = span,
           let spanString = "\(span.latitudeDelta),\(span.longitudeDelta)".urlQueryEncoded {
